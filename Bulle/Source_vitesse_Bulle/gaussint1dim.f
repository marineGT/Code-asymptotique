...
c 	l'intégration d'une fonction 'F' dans 
c 	l'intervalle [a,b]         
c     
c
c     Appelée par le sous programme intepoly pour le calcul
...
c2345678912345678912345678912345678912345678912345678912345678912
      ...
      function gaussint1dim(functionb,a,b,Nt,Nx,c,zetap,hxi,
     &                      dhxi,tabxi,Bn,Dn,alpha,alpha0,b1,
     &                      e1,d1,b2,e2,d2,prec)
c     ==================================================================   
...
c     -------------------------
      ...
      integer Nt,i
      character name*(*)
      parameter (name = 'gaussint1dim')
      dimension wter(12),x(12)
      double precision x,c1,c2,prec
      
c$$$c     Variables utilise par fonctionb.f
c$$$c     ---------------------------------
c$$$      integer :: Nx
c$$$      double precision :: eta, c, zetap
c$$$      double precision :: alpha0
c$$$      double precision, dimension(Nt) :: alpha
c$$$      double precision, dimension(Nt+1) :: Bn, Dn
c$$$      double precision, dimension(Nx) :: hxi, dhxi
c$$$      double precision, dimension(0:Nx) :: tabxi
c$$$      double precision, dimension(Nx) :: b1, e1, d1  
c$$$      double precision, dimension(Nx) :: b2, e2, d2      

c     Declaration des interfaces
c     ---------------------------
c$$$      interface    
c$$$          function fonctionb(eta,Nt,Nx,c,zetap,hxi,dhxi,tabxi,
c$$$     &           Bn,Dn,alpha,alpha0,b1,e1,d1,b2,e2,d2)
c$$$           integer :: Nt,Nx
c$$$           double precision :: eta, c, zetap
c$$$           double precision :: alpha0
c$$$           double precision, dimension(Nt) :: alpha
...
c$$$           double precision, dimension(Nx) :: b1, e1, d1  
c$$$	   double precision, dimension(Nx) :: b2, e2, d2 
c$$$           double precision :: fonctionb
c$$$         end function fonctionb
c$$$       end interface
...
c      WRITE(*,*) 'debut gaussint, c= ', c
 ...
 2    c1=hf*(bb+aa)
      ...
      do 3 i = 1,4
      u=c2*x(i)
      s8=s8+wter(i)*(fonctionb(c1+u,Nt,Nx,c,zetap,hxi,dhxi,
     & tabxi,Bn,Dn,alpha,alpha0,b1,e1,d1,b2,e2,d2)
     & +fonctionb(c1-u,Nt,Nx,c,zetap,hxi,dhxi,tabxi,Bn,Dn,
     & alpha,alpha0,b1,e1,d1,b2,e2,d2))
 3    continue
      s16=0
      do 4 i = 5,12
      u=c2*x(i)
      s16=s16+wter(i)*(fonctionb(c1+u,Nt,Nx,c,zetap,hxi,dhxi,
     & tabxi,Bn,Dn,alpha,alpha0,b1,e1,d1,b2,e2,d2)
     & +fonctionb(c1-u,Nt,Nx,c,zetap,hxi,dhxi,tabxi,Bn,Dn,
     & alpha,alpha0,b1,e1,d1,b2,e2,d2))
 4    continue
